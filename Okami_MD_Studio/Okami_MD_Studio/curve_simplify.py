...
#Original code by testscreenings, Michael Soluyanov
#this is a modified version for Okami Studio
...
def getFcurveData(obj,only_selected):
    fcurves = []
...
class GRAPH_OT_simplify(Operator):
    bl_idname = "graph.simplify_okami"
    ...
    def execute(self, context):
        ...
        if not self.fcurves:
            self.fcurves = getFcurveData(obj, self.only_selected)
...
class Curve_OT_CurveRemvDbs(Operator):
    """Merge consecutive points that are near to each other"""
    bl_idname = 'curve.remove_double_okami'
